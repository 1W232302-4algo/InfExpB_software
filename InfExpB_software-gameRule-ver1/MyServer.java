...
public class MyServer {
    ...
    public static void main(String[] args) {
        ...
        try (ServerSocket serverSocket = new ServerSocket(PORT)) {
            ...
            AtomicInteger maxPlayerId = new AtomicInteger(0);
            ...
            while(running){
                ...
                Thread clientThread = new Thread(() -> {
                    try {
                        ...
                        maxPlayerId.set(Math.max(maxPlayerId.get(), playerNumber.get() % 100));//number of players

                        playerOutputs.put(playerNumber.get() % 100, out);

                        Thread receiveThread = new Thread(() -> {//receive
                            try {
                                if(maxPlayerId.get() < PLAYERS - 1){// wait other players
                                    String inputLine;
                        ...
                        sendTimer.scheduleAtFixedRate(new TimerTask() {//send
                            ...
                            public void run() {
                                for (Map.Entry<Integer, PrintWriter> entry : playerOutputs.entrySet()) {
                                    ...
                                    if (maxPlayerId.get() == PLAYERS - 1){
                                        out.println(gameBoard.BoardData() + gameBoard.players[playerId].getItem() + playerId);
                                    }else{
                                        out.println("starting," + maxPlayerId.get());
                                    }
                                }
                            }
                        }, 0, 100);
    ...
    private static class MyGameBoard {
        ...
        private final int bombTimeShort = -2;//0.2s
        ...
        private final int playerLife = 3;//life is 3
        ...
        public synchronized boolean move(int playerNumber, int x_before, int y_before, int x_after, int y_after) {
            ...
            if(board[x_after][y_after] > bombTimeLong && board[x_after][y_after] < 0) {
                players[playerNumber%100].updatePlayerNumber(playerNumber + 100);
                board[x_after][y_after] = playerNumber + 100;
                board[x_before][y_before] = 0;
                return true;
            }
        ...
        public synchronized void clockPlayerWaitTime(){
            for(int i=0; i<PLAYERS; i++){
                ...
                if(player != null && (int)(player.playerNumber / 100) > 10 + playerLife){
                    player.playerCondition = "dead";
                    board[player.x][player.y] = 0;
                }
                if(player != null && player.playerWaitTime > 0){
                    player.playerWaitTime--;
                }
                if(player != null && player.playerBombWaitTime > 0){
                    player.playerBombWaitTime--;
                }
            }
        }
